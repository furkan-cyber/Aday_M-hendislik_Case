...
db_string="postgresql://postgres:2000@localhost:5432/postgres"
connection=create_engine(db_string).connect()
print(connection)

gps=pd.read_sql_table("gps",connection)
gps.columns=["flight_id","timee","lat","lon","alt"]
print(gps.head())


ang_vel=pd.read_sql_table("ang_vel",connection)
ang_vel.columns=["flight_id","timee","xyz0","xyz1","xyz2","typee"]
print(ang_vel.head())


flight=pd.read_sql_table("flight",connection)
flight.columns=["flight_id","model_id","mission_id","flight_date","landing_date",
